...
// ─────────────────────────────────────────────
//  Sponsor Items (embedded in Rewards tab)
// ─────────────────────────────────────────────
type SponsorItem = {
  id: string;
  name: string;
  ...
  emoji: string;
  coinCost: number;
  type: 'sponsored_post' | 'sponsored_profile';
};

const DEFAULT_SPONSOR_ITEMS: SponsorItem[] = [
  { id: 'sponsored_post_slot', name: 'Spotlight Post', description: 'Your chosen post appears as a glowing purple card in the global feed.', emoji: '📝', coinCost: 500, type: 'sponsored_post' },
  { id: 'sponsored_profile_slot', name: 'Spotlight Profile', description: 'Your full profile appears as a glowing card in the feed.', emoji: '👤', coinCost: 750, type: 'sponsored_profile' },
];

function SponsorItemsSection() {
  const { firestore } = useFirebase();
  const { toast } = useToast();

  const configRef = useMemoFirebase(() => doc(firestore, 'settings', 'rewardsConfig'), [firestore]);
  const { data: config } = useDoc<{ sponsorItems?: SponsorItem[] }>(configRef);

  const [items, setItems] = useState<SponsorItem[]>(DEFAULT_SPONSOR_ITEMS);
  const [saving, setSaving] = useState(false);
  const [editIdx, setEditIdx] = useState<number | null>(null);
  const [addOpen, setAddOpen] = useState(false);
  const [newItem, setNewItem] = useState<SponsorItem>({ id: '', name: '', description: '', emoji: '⚡', coinCost: 500, type: 'sponsored_post' });

  useEffect(() => {
    setItems(config?.sponsorItems ?? DEFAULT_SPONSOR_ITEMS);
  }, [config]);

  const save = async () => {
    setSaving(true);
    try {
      await setDoc(doc(firestore, 'settings', 'rewardsConfig'), { sponsorItems: items }, { merge: true });
      toast({ title: 'Sponsor items saved!' });
    } catch { toast({ variant: 'destructive', title: 'Save failed' }); }
    finally { setSaving(false); }
  };

  const update = (idx: number, field: keyof SponsorItem, value: any) =>
  ...
  const addItem = () => {
    if (!newItem.id || !newItem.name) { toast({ variant: 'destructive', title: 'ID and Name required' }); return; }
    if (items.some(i => i.id === newItem.id)) { toast({ variant: 'destructive', title: 'ID already exists' }); return; }
    setItems(prev => [...prev, { ...newItem }]);
    setNewItem({ id: '', name: '', description: '', emoji: '⚡', coinCost: 500, type: 'sponsored_post' });
    setAddOpen(false);
    toast({ title: 'Item added — remember to save!' });
  };

  const editingItem = editIdx !== null ? items[editIdx] : null;

  const ItemFields = ({ val, onChange }: { val: SponsorItem; onChange: (f: keyof SponsorItem, v: any) => void }) => (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div><label className="text-xs text-muted-foreground block mb-1">ID (unique)</label>
          <Input value={val.id} onChange={e => onChange('id', e.target.value)} className="h-8 text-sm" /></div>
        <div><label className="text-xs text-muted-foreground block mb-1">Emoji</label>
          <Input value={val.emoji} onChange={e => onChange('emoji', e.target.value)} className="h-8 text-sm" /></div>
      </div>
      <div><label className="text-xs text-muted-foreground block mb-1">Name</label>
        <Input value={val.name} onChange={e => onChange('name', e.target.value)} className="h-8 text-sm" /></div>
      <div><label className="text-xs text-muted-foreground block mb-1">Description</label>
        <Textarea value={val.description} onChange={e => onChange('description', e.target.value)} rows={2} className="text-sm" /></div>
      <div className="grid grid-cols-2 gap-3">
        <div><label className="text-xs text-muted-foreground block mb-1">CYBACOIN Cost</label>
          <Input type="number" value={val.coinCost} onChange={e => onChange('coinCost', Number(e.target.value))} className="h-8 text-sm" /></div>
        <div><label className="text-xs text-muted-foreground block mb-1">Type</label>
          <Select value={val.type} onValueChange={v => onChange('type', v)}>
            <SelectTrigger className="h-8 text-sm"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="sponsored_post">Spotlight Post</SelectItem>
              <SelectItem value="sponsored_profile">Spotlight Profile</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
    ...
    <>
      ...
      <Dialog open={editIdx !== null} onOpenChange={open => !open && setEditIdx(null)}>
        <DialogContent className="max-w-lg">
          {editingItem && (<>
            <DialogHeader><DialogTitle>Edit: {editingItem.emoji} {editingItem.name}</DialogTitle></DialogHeader>
            <ItemFields val={editingItem} onChange={(f, v) => update(editIdx!, f, v)} />
            <DialogFooter className="mt-4">
              <Button variant="destructive" onClick={() => del(editIdx!)}>Delete</Button>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>Add Sponsor Item</DialogTitle></DialogHeader>
          <ItemFields val={newItem} onChange={(f, v) => setNewItem(p => ({ ...p, [f]: v }))} />
          <DialogFooter className="mt-4">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold">Spotlight Slots</h3>
            <p className="text-xs text-muted-foreground mt-0.5">
              Items purchasable for CYBACOIN that grant spotlight post/profile inventory slots.
            </p>
          </div>
          <div className="flex gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {items.map((item, idx) => (
            <div key={item.id} className="border border-purple-500/20 rounded-xl p-4 bg-purple-950/10 flex items-start gap-3">
              <span className="text-2xl">{item.emoji}</span>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm">{item.name}</p>
                <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{item.description}</p>
                <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                  <span className="text-yellow-400 font-bold">{item.coinCost.toLocaleString()} CC</span>
                  <span className="capitalize">{item.type.replace('_', ' ')}</span>
                </div>
              </div>
              <Button size="sm" variant="ghost" onClick={() => setEditIdx(idx)}><Edit className="w-4 h-4" /></Button>
            </div>
          ))}
        </div>
      </div>
...
// ─────────────────────────────────────────────
//  Rewards Management (reward-type extras + sponsor items)
// ─────────────────────────────────────────────
